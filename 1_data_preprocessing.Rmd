...
```{r}
# The above SNPs have a missing >10000, we decide to exclude those SNPs (N = 322)
...
info_snp_AD_EUR = snp_match(snp_AD_EUR, map_noNA) # 1545 variants have been matched
```

### A. Independent significant SNPs 
```{r}
final_extract1 = info_snp_AD_EUR %>% filter(ind_sig == 1) %>% unique() # N = 121
dim(final_extract1)
final_extract2 = info_snp_AD_EUR %>% filter(ind_sig == 0) %>% 
  ...
  filter(order == 1) %>% select(-order) # N = 27
dim(final_extract2)
indsig_AD_EUR_final = rbind(final_extract1, final_extract2) %>% unique() %>% 
  select(rsid, chr, pos, a1, a0, beta, gwasP, rsID) 
dim(indsig_AD_EUR_final) # dim = (148,8)
# Remap to get final
info_snp = snp_match(indsig_AD_EUR_final, map_noNA) # 148 variants have been matched
...
### B. Lead SNPs
```{r}
final_extract = info_snp_AD_EUR %>% filter(lead == 1) %>% unique() # N = 38
dim(final_extract)
lead_AD_EUR_final = final_extract %>% 
  select(rsid, chr, pos, a1, a0, beta, gwasP, rsID)
dim(lead_AD_EUR_final) # dim = (38,8)
# Remap to get final
info_snp = snp_match(lead_AD_EUR_final, map_noNA) # 38 variants have been matched
...
map_AD_EUR_final = final_extract %>% 
  select(rsid, chr, pos, a1, a0, beta, gwasP, rsID)
dim(map_AD_EUR_final) # dim = (146,8)
# Remap to get final
info_snp = snp_match(map_AD_EUR_final, map_noNA) # 146 variants have been matched
...
snp_AD_AFR = liftover_full %>% filter(phenotype == "AD_AFR") %>% 
  ...
  mutate(map_yes = case_when(
    ...
    TRUE ~ 0
                ...
                ind_sig, lead, map_yes, CADD) %>% unique()
# Reverse beta if necessary by performing SNP matching
info_snp_AD_AFR = snp_match(snp_AD_AFR, map_noNA) # 74 variants have been matched
```
...
indsig_AD_AFR_final = rbind(final_extract1, final_extract2) %>% unique() %>% 
  select(rsid, chr, pos, a1, a0, beta, gwasP, rsID)
dim(indsig_AD_AFR_final) # dim = (22,8)
# Remap to get final
info_snp = snp_match(indsig_AD_AFR_final, map_noNA) # 22 variants have been matched
...
AD_AFR_indsig_EUR_1kg_mean = AD_EUR_1kg_mean
save(AD_AFR_indsig_EUR_1kg_mean, 
     file = paste0(raw_data_path, "prs/mod/AD_AFR_indsig_EUR_1kg_mean.rda"))
AD_EUR_1kg_sd = sd(onekg_EUR$AD_PRS_AFR_indsig)
AD_AFR_indsig_EUR_1kg_sd = AD_EUR_1kg_sd
save(AD_AFR_indsig_EUR_1kg_sd, 
     file = paste0(raw_data_path, "prs/mod/AD_AFR_indsig_EUR_1kg_sd.rda"))
# Apply to ATLAS
...
# Remap to get final
info_snp = snp_match(lead_AD_AFR_final, map_noNA) # 5 variants have been matched
...
# Remap to get final
info_snp = snp_match(map_AD_AFR_final, map_noNA) # 22 variants have been matched
...
snp_AD_trans = liftover_full %>% filter(phenotype == "AD_trans") %>% 
  ...
  mutate(map_yes = case_when(
    ...
    TRUE ~ 0
                ...
                ind_sig, lead, map_yes, CADD) %>% unique()
# Reverse beta if necessary by performing SNP matching
info_snp_AD_trans = snp_match(snp_AD_trans, map_noNA) # 629 variants have been matched
```
...
indsig_AD_trans_final = rbind(final_extract1, final_extract2) %>% unique() %>% 
  select(rsid, chr, pos, a1, a0, beta, gwasP, rsID)
dim(indsig_AD_trans_final) # dim = (128,8)
# Remap to get final
info_snp = snp_match(indsig_AD_trans_final, map_noNA) # 128 variants have been matched
...
# Remap to get final
info_snp = snp_match(lead_AD_trans_final, map_noNA) # 37 variants have been matched
...
# Remap to get final
info_snp = snp_match(map_AD_trans_final, map_noNA) # 123 variants have been matched
...
snp_LBD = liftover_full %>% filter(phenotype == "LBD") %>% 
  ...
  mutate(map_yes = case_when(
    ...
    TRUE ~ 0
                ...
                ind_sig, lead, map_yes, CADD) %>% unique()
# Reverse beta if necessary by performing SNP matching
info_snp_LBD = snp_match(snp_LBD, map_noNA) # 304 variants have been matched
```
...
# Remap to get final
info_snp = snp_match(indsig_LBD_final, map_noNA) # 21 variants have been matched
...
# Remap to get final
info_snp = snp_match(lead_LBD_final, map_noNA) # 7 variants have been matched
...
# Remap to get final
info_snp = snp_match(map_LBD_final, map_noNA) # 21 variants have been matched
...
snp_PD = liftover_full %>% filter(phenotype == "PD") %>% 
  ...
  mutate(map_yes = case_when(
    ...
    TRUE ~ 0
                ...
                ind_sig, lead, map_yes, CADD) %>% unique()
# Reverse beta if necessary by performing SNP matching
info_snp_PD = snp_match(snp_PD, map_noNA) # 1760 variants have been matched
```

### A. Independent significant SNPs 
```{r}
final_extract1 = info_snp_PD %>% filter(ind_sig == 1) %>% unique() # N = 53
dim(final_extract1)
...
indsig_PD_final = rbind(final_extract1, final_extract2) %>% unique() %>% 
  select(rsid, chr, pos, a1, a0, beta, gwasP, rsID)
dim(indsig_PD_final) # dim = (69,8)
# Remap to get final
info_snp = snp_match(indsig_PD_final, map_noNA) # 69 variants have been matched
...
### B. Lead SNPs
```{r}
final_extract = info_snp_PD %>% filter(lead == 1) %>% unique() # N = 17
dim(final_extract)
lead_PD_final = final_extract %>% 
  select(rsid, chr, pos, a1, a0, beta, gwasP, rsID)
dim(lead_PD_final) # dim = (17,8)
# Remap to get final
info_snp = snp_match(lead_PD_final, map_noNA) # 17 variants have been matched
...
map_PD_final = final_extract %>% 
  select(rsid, chr, pos, a1, a0, beta, gwasP, rsID)
dim(map_PD_final) # dim = (69,8)
# Remap to get final
info_snp = snp_match(map_PD_final, map_noNA) # 69 variants have been matched
...
snp_PSP = liftover_full %>% filter(phenotype == "PSP") %>% 
  ...
  mutate(map_yes = case_when(
    ...
    TRUE ~ 0
                ...
                ind_sig, lead, map_yes, CADD) %>% unique()
# Reverse beta if necessary by performing SNP matching
info_snp_PSP = snp_match(snp_PSP, map_noNA, match.min.prop = 0) # 554 variants have been matched
```
...
indsig_PSP_final = rbind(final_extract1, final_extract2) %>% unique() %>% 
  select(rsid, chr, pos, a1, a0, beta, gwasP, rsID)
dim(indsig_PSP_final) # dim = (35,8)
# Remap to get final
info_snp = snp_match(indsig_PSP_final, map_noNA) # 35 variants have been matched
...
# Remap to get final
info_snp = snp_match(lead_PSP_final, map_noNA) # 13 variants have been matched
...
# Remap to get final
info_snp = snp_match(map_PSP_final, map_noNA) # 35 variants have been matched
...
snp_STROKE = liftover_full %>% filter(phenotype == "STROKE") %>% 
  ...
  mutate(map_yes = case_when(
    ...
    TRUE ~ 0
                ...
                ind_sig, lead, map_yes, CADD) %>% unique()
# Reverse beta if necessary by performing SNP matching
info_snp_STROKE = snp_match(snp_STROKE, map_noNA) # 677 variants have been matched
```

### A. Independent significant SNPs 
```{r}
final_extract1 = info_snp_STROKE %>% filter(ind_sig == 1) %>% unique() # N = 38
dim(final_extract1)
...
indsig_STROKE_final = rbind(final_extract1, final_extract2) %>% unique() %>% 
  select(rsid, chr, pos, a1, a0, beta, gwasP, rsID)
dim(indsig_STROKE_final) # dim = (46,8)
# Remap to get final
info_snp = snp_match(indsig_STROKE_final, map_noNA) # 46 variants have been matched
...
### B. Lead SNPs
```{r}
final_extract = info_snp_STROKE %>% filter(lead == 1) %>% unique() # N = 16
lead_STROKE_final = final_extract %>% 
  select(rsid, chr, pos, a1, a0, beta, gwasP, rsID)
dim(lead_STROKE_final) # dim = (16,8)
# Remap to get final
info_snp = snp_match(lead_STROKE_final, map_noNA) # 16 variants have been matched
...
# Remap to get final
info_snp = snp_match(map_STROKE_final, map_noNA) # 39 variants have been matched
...
dim(atlas_prs_final) # dim = c(27948,26)
...
length(AD.indsig.combine.id) # 236
...
length(all.indsig.combine.id) # 391
...
length(all.lead.combine.id) # 115
